...
def read_dataframe(file):
    df = pd.read_csv(file, sep='\t', usecols=['variant_id', 'pheno_id'],memory_map=True)
    return df
...
if chunks is None:
    for f in file_paths:
        print(f)
        dfs.append(pd.read_csv(f, sep='\t', usecols=['variant_id', 'pheno_id'],memory_map=True))
...
dfs = dfs.sort_values(['chr', 'pos', 'pheno_id'])
