...
ref_pairs_df = pd.read_csv(args.extract_pairs, sep='\t', usecols=['variant_id', 'pheno_id', 'chr'], dtype={'variant_id':str, 'pheno_id':str, 'chr':int}, memory_map=True)
if args.chrom is not None:
    print(args.chrom)
    ref_pairs_df = ref_pairs_df[ref_pairs_df['chr']==args.chrom]
ref_pair_ids = set(ref_pairs_df['variant_id']+','+ref_pairs_df['pheno_id'])
...
for f in file_paths:
    ...
    for i,chunk in enumerate(pd.read_csv(f, sep='\t', iterator=True, chunksize=1000000, index_col=1,
        dtype={'pheno_id':str, 'variant_id':str, 'tss_distance':np.int32,
            'ma_samples':np.int32, 'ma_count':np.int32, 'af':np.float32,
            'pval_g1':np.float64, 'beta_g1':np.float32, 'beta_se_g1':np.float32},memory_map=True)):

        ix = (chunk.index.values+','+chunk['pheno_id']).apply(lambda x: x in ref_pair_ids)
...
signif_df.index = signif_df['variant_id']+','+signif_df['pheno_id']

print('\nSorting extracted pairs.')
output_df = pd.DataFrame(index=ref_pairs_df['variant_id']+','+ref_pairs_df['pheno_id'])
output_df = output_df.join(signif_df[['pval_g1', 'beta_g1', 'beta_se_g1']])
