...
def load_pair_data(path):
    if path.endswith('_na_extracted_pairs.txt'):
        return pd.read_csv(path, sep='\t', usecols=['pair_id', 'beta_g1', 'beta_se_g1'], index_col=0, dtype={'pair_id':str, 'beta_g1':np.float32, 'beta_se_g1':np.float32}, memory_map=True)
...
sample_ids = np.array([os.path.split(i)[1].split("_na_extracted_pairs.txt")[0] for i in paths])
print(sample_ids)
